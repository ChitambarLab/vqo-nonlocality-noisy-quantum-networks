...
def star_nlocal_max_entangled_prep_nodes(n):
    return [
        QNopt.PrepareNode(1, [i, n+i], max_entangled, 3) for i in range(n)
    ]


def star_22_local_ry_meas_nodes(n):
    meas_nodes = [
        QNopt.MeasureNode(2, 2, [i], QNopt.local_RY, 1) for i in range(n)
    ]
    meas_nodes.append(
        QNopt.MeasureNode(2, 2, [i for i in range(n,2*n)], QNopt.local_RY, n)
    )
...
def star_22_local_rot_meas_nodes(n):
    meas_nodes = [
        QNopt.MeasureNode(2, 2, [i], local_rot, 3) for i in range(n)
    ]
    meas_nodes.append(
        QNopt.MeasureNode(2, 2, [i for i in range(n,2*n)], local_rot, 3*n)
    )
