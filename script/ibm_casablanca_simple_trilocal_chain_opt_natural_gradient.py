...
prep_nodes = [
    ...
    QNopt.PrepareNode(1, [4, 5], QNopt.ghz_state, 0),
]
...
"""
Theoretical execution (shots undefined) works for the natural gradient,
but when shots is defined, the natural gradient does not optimize well.

If this trend continues for the star network, we should discard natural
gradient as a viable means of optimization.
"""
dev_ibm = {
    # "name": "qiskit.ibmq",
    "name" : "default.qubit",
...
nat_grad = QNopt.parallel_nlocal_chain_grad_fn(
    ibm_ansatz, natural_gradient=False, diff_method="parameter-shift"
)

data_filepath = "script/data/ibm_casablanca_simple_trilocal_chain_opt_natural_gradient/"
