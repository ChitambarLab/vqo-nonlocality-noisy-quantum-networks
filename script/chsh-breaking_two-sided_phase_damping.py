...
def chsh_noise_optimization(prep_nodes, meas_nodes, **opt_kwargs):
    def _optimization_fn(arg1, arg2):
        ...
        noise_nodes = [
            QNopt.NoiseNode([0], lambda settings, wires: qml.PhaseDamping(arg1, wires=wires[0])),
            QNopt.NoiseNode([1], lambda settings, wires: qml.PhaseDamping(arg2, wires=wires[0])),
        ]
...
def save_optimization_results(ansatz_name, x_range, y_range, opt_dicts):
    ...
    filename = (
        "script/data/chsh-breaking_two-sided_phase_damping/" + ansatz_name + "/" + datetime_ext
    )
...
if __name__ == "__main__":
    ...
    max_entangled_optimization = chsh_noise_optimization(
        max_entangled_prep, meas_nodes, sample_width=5, step_size=0.15, num_steps=40, verbose=False
    )
    ...
    arb_optimization = chsh_noise_optimization(
        arb_prep, meas_nodes, sample_width=5, step_size=0.15, num_steps=40, verbose=False
    )
