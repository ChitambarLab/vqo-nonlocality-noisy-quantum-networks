...
if __name__ == "__main__":
	...
	chsh_ideal_scores = [
	    -(chsh_cost(chsh_data["opt_settings"][i])) for i in range(num_chsh_steps)
	]
	...
	star_ideal_scores = [
	    -(star_cost(star_data["opt_settings"][i])) for i in range(num_star_steps)
	]
	...
	for i in range(4):
		...
		ax.plot(range(num_steps), [quantum_bounds[i]]*num_steps, "-", linewidth=2, label="Quantum Bound")
		ax.plot(range(num_steps), [classical_bounds[i]]*num_steps, "--", linewidth=2, label="Classical Bound")
		ax.plot(range(num_steps), ideal_data_sets[i], ":d", markersize=8, linewidth=2, label="Max Noiseless Optimized Score")
		ax.plot(range(num_steps), data_sets[i]["max_scores"], ":o", markersize=7 , linewidth=2, label="Max Optimized Score")
		ax.errorbar(
		    range(num_steps),
