...
The considered noise model is an phase damping channel to a single
...
def single_qubit_phase_damping_nodes_fn(n, wire):
    def noise_nodes(noise_args):
        return [
            qnet.NoiseNode(
                [wire,2*n], lambda settings, wires: qnet.pure_phase_damping([noise_args], wires=wires)
            )
        ]
...
if __name__ == "__main__":
    ...
    data_dir = "data/n-star/single_qubit_phase_damping/"
    ...
    for n in [3,4]:
        ...
        for wire in [0,n]:
            ...
            """
            Minimal optimal ansatz for phase damping
            """
