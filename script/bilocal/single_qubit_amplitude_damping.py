...
def single_qubit_amplitude_damping_nodes_fn(wire):
    def noise_nodes(noise_args):
        return [
            qnet.NoiseNode(
                [wire,4+wire], lambda settings, wires: qnet.pure_amplitude_damping([noise_args], wires=wires)
            )
        ]
...
if __name__ == "__main__":
    ...
    data_dir = "data/bilocal/single_qubit_amplitude_damping/"
    ...
    for wire in [0, 1]:
        ...
        max_ent_local_rot_opt = src.noisy_net_opt_fn(
            max_ent_prep_nodes,
            local_rot_meas_nodes,
            single_qubit_amplitude_damping_nodes_fn(wire),
